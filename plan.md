...
- [ ] Modified File Rewrite
    - [ ] Goal: improve post-merge, post-rewrite, pre-push, post-checkout hooks performance by only looping through modified `.FCStd` files instead of ALL files.
		- [ ] Add `.filepath` file that contains path to `.FCStd` file
		- [ ] Merge list of changed files with same FCStd_dir_path in hook
		- [ ] Iterate through list of changed FCStd_dir_path(s) instead
			- [ ] Potential Edge case => change is a deletion
