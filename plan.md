...
IDEA:
1. Git clean filter makes .FCStd files look empty (from git's pov)
2. Users lock a .lockfile inside the `_FCStd` dir instead of the .FCStd file itself
3. On Post-Checkout Hook
	- Update .FCStd files with uncompressed files 
	- if the .lockfile is not locked by the user, the .FCStd file is set to readonly.
4. On Pre-Commit Hook:
	- Extracts non-locked .FCStd files
5. On Pre-Push Hook:
	- If locked dir changed, cancel push
6. To lock, use lock.sh path/to/file.FCStd | path/to/.lockfile:
	- Set git lfs lock appropriate _FCStd directory .lockfile
	- mark .FCStd file as writable
7. To unlock, use unlock.sh path/to/file.FCStd | path/to/.lockfile:
